...
def reference_member_inference(member_data, non_member_data, granularity):
    ...
    label_list = []
    for _ in range(1000):
        samples_reference_member = random.sample(reference_member, granularity)
        ...
        mean_reference_member, mean_target_member, mean_target_non_member = np.mean(samples_reference_member), np.mean(samples_target_member), np.mean(samples_target_non_member)
        var_reference_member, var_target_member, var_target_non_member = np.var(samples_reference_member, ddof=1), np.var(samples_target_member, ddof=1), np.var(samples_target_non_member, ddof=1)
        
        z_member = (mean_target_member - mean_reference_member) / np.sqrt(var_target_member / len(samples_target_member) + var_reference_member / len(samples_reference_member))
        p_member = 1- norm.cdf(z_member)
        p_list.append(p_member)
        label_list.append(1)
        
        z_non_member = (mean_reference_member - mean_target_non_member) / np.sqrt(var_reference_member / len(samples_reference_member) + var_target_non_member / len(samples_target_non_member))
...
def main(args):
    ...
    for _ in range(5):
        auc = reference_member_inference(member_data, non_member_data, args.granularity)
        aucs.append(auc)
